...
class RedLineDetectionController:
    ...
    def prepare_learned_path(self):
        if not self.optimal_path:
            return
        self.path_waypoints = []
        for i in range(0, len(self.optimal_path), 15):  
    ...
    def wall_following_adjustment(self, ds0_val, ds4_val):
        ...
        if ds0_val < self.WALL_THRESHOLD and ds4_val > self.WALL_THRESHOLD:
            # Wall detected on right - turn left (away from wall)
            wall_intensity = (self.WALL_THRESHOLD - ds0_val) / self.WALL_THRESHOLD
            left_adj -= self.WALL_K * wall_intensity
            right_adj += self.WALL_K * wall_intensity

        # Check left wall (ds4)
        elif ds4_val < self.WALL_THRESHOLD and ds0_val > self.WALL_THRESHOLD:
            # Wall detected on left - turn right (away from wall)
            wall_intensity = (self.WALL_THRESHOLD - ds4_val) / self.WALL_THRESHOLD
            left_adj += self.WALL_K * wall_intensity
            right_adj -= self.WALL_K * wall_intensity
        elif ds4_val < self.WALL_THRESHOLD and ds0_val < self.WALL_THRESHOLD:
            if ds0_val < ds4_val:
    ...
    def obstacle_detection_adjustment(self, ds1_val, ds2_val, ds3_val):
        ...
        ds1_triggered = ds1_val < self.TURN_THRESHOLD
        ds3_triggered = ds3_val < self.TURN_THRESHOLD

        if ds1_triggered and not ds3_triggered:
            # Obstacle on front-right, turn left
            obstacle_intensity = (self.OBSTACLE_THRESHOLD - ds1_val) / self.OBSTACLE_THRESHOLD
            ...
            obstacle_intensity = (self.OBSTACLE_THRESHOLD - ds3_val) / self.OBSTACLE_THRESHOLD
            left_adj += self.OBSTACLE_K * obstacle_intensity
            right_adj -= self.OBSTACLE_K * obstacle_intensity
            ...
            if ds1_val > ds3_val:
                ...
                left_adj -= self.OBSTACLE_K * obstacle_intensity
                right_adj += self.OBSTACLE_K * obstacle_intensity
        elif ds2_val < self.OBSTACLE_THRESHOLD:
