 ...
 * - Manual control between rounds (no auto-reset)
 ...
 * 3. After finish, manually start next round with 'NEXT' command or place on start line
 * 4. Manual commands: "RESET", "NEXT", "STATUS", "HELP"
 * 
 ...
 */
 ...
 void setup() {
   ...
   Serial.println("   🔧 MANUAL CONTROL: Each round requires manual start");
   Serial.println();
   Serial.println("📍 INSTRUCTIONS:");
   Serial.println("   1. Place robot on RED START LINE");
   Serial.println("   2. Robot will automatically detect line and start");
   Serial.println("   3. After each round, manually start next round:");
   Serial.println("      - Type 'NEXT' or 'N' in serial monitor");
   Serial.println("      - OR place robot on start line again");
   Serial.println("======================================================");
   Serial.println();
 ...
 void loop() {
   ...
   if (Serial.available()) {
     ...
     if (command == "RESET" || command == "R") {
       ...
       if (waiting_for_next_round && current_round < MAX_ROUNDS) {
         startNextRound();
       } else {
         Serial.println("❌ No next round available. Use 'STATUS' to check current state.");
       }
       return;
       ...
       Serial.println("   NEXT or N - Start next round (when available)");
     ...
     case RACE_FINISHED:
       ...
       if (!waiting_for_next_round) {
         ...
         if (current_round < MAX_ROUNDS) {
           ...
           Serial.println("   🔧 MANUAL CONTROL: Next round requires manual start");
           Serial.println("   📋 Options to start next round:");
           Serial.println("     1. Type 'NEXT' or 'N' in serial monitor");
           Serial.println("     2. Place robot on RED START LINE (auto-detect)");
           Serial.println("     3. Type 'RESET' to restart from Round 1");
         } else {
           Serial.println();
 ...
 // Manual start next round function
 void startNextRound() {
   if (!waiting_for_next_round || current_round >= MAX_ROUNDS) {
     Serial.println("❌ No next round available!");
     return;
   }
   
   Serial.println();
   Serial.print("🔄 MANUAL START: Starting Round ");
   Serial.print(current_round + 1);
   Serial.print("/");
   Serial.println(MAX_ROUNDS);
   Serial.println("   Place robot on RED START LINE to begin");
   ...
   changeState(WAITING_START);
   Serial.println("⏳ Waiting for RED START LINE detection...");
 }
